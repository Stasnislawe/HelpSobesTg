...
async def create_learning_path(user_telegram_id: int, topics: list, title: str = None) -> int:
    ...
    async with AsyncSessionLocal() as session:
        # Деактивируем все старые активные планы этого пользователя
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(LearningPath).where(
                LearningPath.user_id == user.id,
                LearningPath.is_active == True
            ).order_by(LearningPath.created_at.desc()).limit(1)   # берём самый свежий активный план
        )
        return result.scalar_one_or_none()
...
async def get_learning_path_by_id(path_id: int) -> LearningPath | None:
    async with AsyncSessionLocal() as session:
